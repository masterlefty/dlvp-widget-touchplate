...
cpdefine("inline:com-chilipeppr-dlvp-widget-touchplate", ["chilipeppr_ready", /* other dependencies here */ ], function() {
    return {
        ...
        onRun: function(evt) {
            ...
            // Unsubscribe to coordinates and units so widget can recall settings
            ...
            if (this.isRunning) {
                ...
                if (runCode == 'run3') { // run3 runs from feedrate on run2
                    var fr = $('#' + this.id + ' .frprobe-run2').val();
                }
                else if (runCode == 'run5') { // run5 runs from feedrate on run4
                    var fr = $('#' + this.id + ' .frprobe-run4').val();
                }
                else { // run1, run2, and run 4 run direct from runCode
                ...
                /**
                 * Note: testing has shown that the g91 throws a bug into 
                 * the coordinates it will add the current mpo position to the 
                 * G30.1 saved coordinates and present them back to WCS 
                 * coordinates. Break the Z-clearance move into a separate step
                 */
                
                // move to defined G30 location
                if (runCode == "run4" || runCode == "run5") {
                    console.log("G30 runCode was triggered by:", runCode);
                    ...
                    // If initial units were inch, return to G20 mode
        ...
        onG30: function(evt) {
            ...
            if (this.isRunning) {
                ...
                // Set Units as stored in global definition gcodeUnit
                var id = "tp" + this.gcodeCtr++;
                var gcode = gcodeUnit + "\n";
                chilipeppr.publish("/com-chilipeppr-widget-serialport/jsonSend", {Id: id, D: gcode});
        ...
        afterProbeDone: function(probeData) {
             ...
             * Probe command runs under machine base units [mm]
             * probeData.z is always based in mm
             * Need to check initial Unit definition and if is
            ...
            // Sets the WCS (g5x) coordinate system for buttons 2 or 4
                ...
                // Button 3 on Tab 2 is for use with a floating touchplate
                // Button 5 on Tab 3 is for use with a fixed touchplate
            ...
            // Need to check units, inch or mm
                // (16.02.28) backoff in inches was changed from 0.1 to 0.2 due to controller 
